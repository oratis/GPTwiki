...
### 9.5 开放匿名对战后，第一场真实对战立刻暴露了第二个原因

打开匿名入口后跑的**第一场真实对战就失败了**——slot A 报错：

```
[GoogleGenerativeAI Error] gemini-2.0-flash:streamGenerateContent
[400 Bad Request] Invalid value at 'system_instruction' (…v1beta.Content)
```

`streamGemini` 把 `systemInstruction` 当**普通字符串**传给了 `startChat`。SDK 的 `StartChatParams` 类型写的是 `string | Part | Content`，但**只有 `GenerativeModel` 构造函数**会跑 `formatSystemInstruction`（`dist/index.js:1362`）；`startChat` 把调用方的参数**最后展开**覆盖掉已格式化的值（`:1401`），且不做任何处理。**所以一个字符串对编译器合法、对服务端畸形**，TypeScript 抓不到。

影响范围远超 arena：`streamGemini` 是全站唯一的 Gemini 通道，`/api/chat`、wiki 跟帖、arena 对战**全部走它**。也就是说 **Gemini 在全站已经坏了不知道多久**。

对 §9 的意义是：**擂台空着有两个独立原因，而不是一个。**

| 原因 | 后果 |
|---|---|
| 硬 401（§9.4） | 没有登录的人**进不来** |
| Gemini 400（本节） | 就算进来了，三种配对里有**两种**含 Gemini ⇒ **约 2/3 的对战必然失败**，且失败的对战**不写文档、不可投票** |

两个都修掉之后，对战链路才第一次真正可用。这也解释了为什么 §9.2 里那 5 次 401 从来没人跟进——即使当时有人登录重试，他有三分之二的概率撞上一个只会失败的对战，而失败是静默的（前端只显示「该模型未能作答」）。

> 教训：**「没人用」和「用了但坏了」在指标上都是零。** §9.2 只查到了「零对战」，据此推断的是漏斗问题；真正跑一次才发现底下还压着一个 100% 复现的后端故障。**在从「零使用量」推断原因之前，先自己走一遍那条路。**
