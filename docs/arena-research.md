...
**结论可信度**：流水线结构（预筛 → 5 维打分 → 代码算总分 → 分级阈值 → 聚类 → 日报）有作者原文支撑，可信。**具体 5 个维度的名称、权重系数、阈值数值属作者明确声明不公开的部分**，本文不做猜测。`/leaderboard/rules` 页若含额外规则（例如「低粉爆文」入榜条件的具体数值），未被覆盖——待该页可达（或有人贴出原文）后再补。
...
**关键设计**：`rank` 对外，**热度值本身不对外公开**（原文：热度值仅内部使用）。给序不给分——这样既可用，又不暴露可被反推的打分函数。
